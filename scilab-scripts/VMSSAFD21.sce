...
vecV=[-110:10:50]
Iinf=[-68.6 -49.5 -18.2 -5.06 2.19 3.37 2.54 2.68 5.97 14.6 33.4 60.2 85 114 154 208 254]

for i=1:length(vecV)
    plot(vecV(i),Iinf(i),'b+')
end
...
function y=W(pa)
    e=0;
    for i=1:length(vecV)
        e=e+(Iinf(i)-(pa(1)*xinf(vecV(i),pa(7),pa(10))*(vecV(i)-pa(4))+pa(2)*xinf(vecV(i),pa(8),pa(11))*xinf(vecV(i),pa(9),pa(12))*(vecV(i)-pa(5))+pa(3)*(vecV(i)-pa(6))))^2
    end
    y=e
endfunction
...
function [valBest]=simulation(NP,itermax,F,CR)
    ...
    Xmin=[0.1 0.1 0.1 30 -90 -80 -90 -90 -90 1 1 -30];
    Xmax=[30 30 30 140 -2 30 -2 -2 -2 30 30 -1];
