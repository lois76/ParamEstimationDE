...
while nbSimStarted <= endingIndex:
	...
	command = ["docker run --name scilab"+str(nbSimStarted)+" --rm  -v $(pwd)/scilab-scripts:/scilab-scripts -v $(pwd)/Results/"+resultPath+":/Result scilab_v2 "+scilabScriptName+" "+str(nbPop)+" "+str(nbIter)+" "+str(mutation)+" "+str(crossover)+ " > outputs/output_docker_" + str(nbSimStarted) + " 2> outputs/output_docker_error_" + str(nbSimStarted)]
