...
tx1=0.8; tx2=6.7; tx3=0.1; tx4=3.4;
x10=0.8; x20=0.36; x30=0.55; x40=0.1;
C=0.6;
...
function [Hdot]=HH11ref(t,x)
    Hdot=zeros(5,1);  //définition de la taille
    Hdot(1)=(1/C)*(-gCa*x(2)*x(3)*(x(1)-ECa)-gK*x(4)*x(5)*(x(1)-EK)-gL*(x(1)-EL)+I)
...
for i=1:11
    I=stim(i);
    x=ode([-40; x10; x20; x30; x40],t0,t,HH11ref);
    x1=x(1,:);
    x1=x1';
    a(:,i)=x1;
end
...
function [bM, valBest]=simulation(NP,itermax,F,CR)
    ...
    Xmin=[0.1 0.1 0.1 20  -90 -80 -90 -90 -90 -90  1  -30  1  -30 0.0001 0.0001 0.0001 0.0001 0.001 0.001 0.001 0.001 0.001];
