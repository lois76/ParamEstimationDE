...
function y=Iinf(VH)
    y = gCa.*xinf(VH,V12mCa,kmCa).*xinf(VH,V12hCa,khCa).*(VH-ECa) + gK.*xinf(VH,V12mK,kmK).*(VH-EK) + gL.*(VH-EL)
endfunction
...
function y=W12(pa)
    e=0;
    for i=1:length(vecV)
        e=e+(Iinf(vecV(i))-(pa(1)*xinf(vecV(i),pa(7),pa(10))*xinf(vecV(i),pa(8),pa(11))*(vecV(i)-pa(4))+pa(2)*xinf(vecV(i),pa(9),pa(12))*(vecV(i)-pa(5))+pa(3)*(vecV(i)-pa(6))))^2
    end
    y=e/length(vecV) 
endfunction
...
//    costVec=zeros(1,itermax);
    ...
    Xmin=[0.1 0.1 0.1 20  -100 -90 -90 -90 -90 1  -30 1];
    Xmax=[50  50  50  150 -2   30  -2  -2  -2  30 -1  30];
