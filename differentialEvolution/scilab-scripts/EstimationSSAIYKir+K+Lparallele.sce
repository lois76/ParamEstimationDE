...
function y=W(pa)
    e=0;
    for i=1:length(vecV)
        e=e+(Inf(i)-(pa(1)*xinf(vecV(i),pa(6),pa(8))*(vecV(i)-pa(4)) + pa(2)*xinf(vecV(i),pa(7),pa(9))*(vecV(i)-pa(4)) + pa(3)*(vecV(i)-pa(5))))^2
    end
    y=e/length(vecV) 
endfunction
...
//    costVec=zeros(1,itermax);
    ...
    Xmin=[0.1 0.1 0.1 -100 -90 -90 -90 -30  1];
    Xmax=[50  50  50  -2   30  -2  -2  -1   30];
