...
A=csvRead("/scilab-scripts/6differentRampWithStabilization.txt");
//A=csvRead("/home/naudin/Documents/article_ramp_current/Fichier Scilab/6differentRampWithStabilization.txt");
...
// Ramp current from stim_start to stim_fin during duration with stabilization during other duration times
function y=Iramp(stim_start,stim_fin,duration,t)
    a = (stim_fin-stim_start)/duration
    if t <= duration then
        y = a*t + stim_start
    else 
        y = stim_fin
...
function [Hdot]=HHramp(t,x,pa,stim_start,stim_fin,duration)
    Hdot=zeros(4,1);
    Hdot(1)=(1/pa(22))*(-pa(1)*x(2)*(x(1)-pa(5)) - pa(2)*xinf(x(1),pa(9),pa(13))*(x(1)-pa(6)) - pa(3)*x(3)*x(4)*(x(1)-pa(6)) - pa(4)*(x(1)-pa(7)) + Iramp(stim_start,stim_fin,duration,t))
...
stim_startVec=[-15 0 15 35 20 5];
stim_finVec=[0 15 35 25 5 -10];
duration=50;

function y=fct11(pa)
    c=0;
    condini = [-33; pa(19); pa(20); pa(21)]
    for i=1:length(stim_startVec)
        stim_start=stim_startVec(i);
        stim_fin=stim_finVec(i);
        x=ode(condini,t0,t,HHramp); 
        V=x(1,:);
        for k=1:100
...
function [bM, valBest]=simulation(NP,itermax,F,CR)
    ...
    Xmax=[10      10      10      10      150 -2   30  -2  -2  -2  -2  30 -1  30 -1  20     20     20     0.999 0.999 0.999 10];
