...
function [Hdot]=HH11(t,x,pa)
    Hdot=zeros(4,1);
    Hdot(1)=(1/pa(20))*(-pa(1)*x(2)*x(3)*(x(1)-pa(4)) - pa(2)*xinf(x(1),pa(8),pa(12))*(x(1)-pa(5)) - pa(3)*x(4)*(x(1)-pa(5)) + I)
    Hdot(2)=(xinf(x(1),pa(6),pa(10))-x(2))/pa(14)
    Hdot(3)=(xinf(x(1),pa(7),pa(11))-x(3))/pa(15)
    Hdot(4)=(xinf(x(1),pa(9),pa(13))-x(4))/pa(16)
endfunction

//Fonction coût 
function y=fct11(pa)
    c=0;
    condini = [-53; pa(17); pa(18); pa(19)]
    for i=1:11
        I=stim(i);
...
function [bM, valBest]=simulation(NP,itermax,F,CR)
    ...
    Xmin=[0.1 0.1 0.1 20  -100 -90 -90 -90 -90 1  -30 -30 1  0.0001 0.0001 0.0001 0.001 0.001 0.001 0.001];
    Xmax=[50  50  50  150 -2   -2  -2  -2  -2  30 -1  -1  30 15     15     15     0.999 0.999 0.999 10];
