	...
	for row in csv_reader:
		...
		if line_count == 0:
			...
			while nbSimPerParam < maxNbSimPerParam:
				...
				Popen("mkdir Results/"+resultPath, shell = True)
				command = ["docker run --name scilab"+str(simId)+"--rm  -v $(pwd)/scilab-scripts:/scilab-scripts -v $(pwd)/Results/"+resultPath+":/Result scilab "+scilabScriptName+" "+row[1]+" "+row[2]+" "+row[3]+" "+row[4]]
